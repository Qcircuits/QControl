...
class SetRFFrequencyTask(InterfaceableTaskMixin, InstrumentTask):
    ...
    driver_list = ['AgilentE8257D', 'AnritsuMG3694', 'LabBrickLMS103',
                                   'RhodeSchwartzSGMA','HewlettPackard8276A','RohdeAndSchwarzSignalGenerators']
...
class SetRFModulationTask(InterfaceableTaskMixin, InstrumentTask):
    ...
    task_database_entries = set_default({'pm_state': 0})
    loopable = True
    driver_list = ['RhodeSchwartzSGMA']
    def check(self, *args, **kwargs):
        """Validate the value of the switch.

        """
        test, traceback = super(SetRFModulationTask, self).check(*args,
                                                                    **kwargs)
